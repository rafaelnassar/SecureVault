...
import { CryptoKeyData } from '@/lib/vault';
import { showToast } from '@/components/ui/sonner';

interface CryptoShareFormatSelectorProps {
  open: boolean;
  cryptoKey: CryptoKeyData | null;
  onClose: () => void;
}
...
export function CryptoShareFormatSelector({ open, cryptoKey, onClose }: CryptoShareFormatSelectorProps) {
  ...
  const generateText = (format: ShareFormat): string => {
    const { name, login, walletAddress, privateKey, seedPhrase, recoveryWords, notes } = cryptoKey;
    
    const formatRecoveryWords = (words: string[], format: ShareFormat) => {
      if (format === 'discord') {
    ...
    switch (format) {
      case 'common':
        return `🔐 Dados da Carteira

📍 Nome: ${name}
${login ? `👤 Login: ${login}\n` : ''}💳 Endereço: ${walletAddress}
${privateKey ? `🔑 Chave Privada: ${privateKey}\n` : ''}${seedPhrase ? `📝 Frase de Recuperação: ${seedPhrase}\n` : ''}${recoveryWords && recoveryWords.length > 0 ? `🔢 Palavras de Recuperação:\n${formatRecoveryWords(recoveryWords, 'common')}\n` : ''}${notes ? `📋 Notas: ${notes}\n` : ''}
...
📍 Nome: ||${name}||
${login ? `👤 Login: ||${login}||\n` : ''}💳 Endereço: ||${walletAddress}||
${privateKey ? `🔑 Chave Privada: ||${privateKey}||\n` : ''}${seedPhrase ? `📝 Frase de Recuperação: ||${seedPhrase}||\n` : ''}${recoveryWords && recoveryWords.length > 0 ? `🔢 Palavras de Recuperação:\n${formatRecoveryWords(recoveryWords, 'discord')}\n` : ''}${notes ? `📋 Notas: ||${notes}||\n` : ''}
...
📍 *Nome:* ${name}
${login ? `👤 *Login:* ${login}\n` : ''}💳 *Endereço:* \`${walletAddress}\`
${privateKey ? `🔑 *Chave Privada:* \`${privateKey}\`\n` : ''}${seedPhrase ? `📝 *Frase de Recuperação:* \`${seedPhrase}\`\n` : ''}${recoveryWords && recoveryWords.length > 0 ? `🔢 *Palavras de Recuperação:*\n${formatRecoveryWords(recoveryWords, 'whatsapp')}\n` : ''}${notes ? `📋 *Notas:* ${notes}\n` : ''}
⚠️ _Mantenha estas informações em segurança!_`;
  ...
  const handleCopy = async () => {
    ...
    setCopied(true);
    showToast.copied('Dados da carteira');
    setTimeout(() => {
      setCopied(false);
      onClose();
    }, 1200);
      ...
      <AnimatePresence mode="wait">
        {copied ? (
          ...
          >
            ...
            {/* Preview */}
            <div className="bg-muted/50 rounded-xl p-3 mb-5 border border-border max-h-[200px] overflow-y-auto">
