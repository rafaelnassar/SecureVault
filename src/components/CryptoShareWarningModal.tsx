...
interface CryptoShareWarningModalProps {
  open: boolean;
  walletName: string;
  onConfirm: () => void;
  onCancel: () => void;
...
export function CryptoShareWarningModal({ open, walletName, onConfirm, onCancel }: CryptoShareWarningModalProps) {
  return (
    <Modal open={open} onClose={onCancel} showCloseButton={false}>
      ...
      >
        ...
        <p className="text-sm text-muted-foreground mb-6">
          Você está prestes a compartilhar os dados da carteira{' '}
        ...
        <div className="flex items-start gap-2 text-xs text-muted-foreground bg-destructive/5 rounded-lg p-3 border border-destructive/20 mb-6">
          ...
          <p className="text-left">
            <strong className="text-destructive">Aviso:</strong> Compartilhar chaves privadas ou frases de recuperação pode resultar na perda total dos seus fundos. Nunca compartilhe com desconhecidos.
          </p>
        </div>
